      ...
      try {
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            // The configured bot owner (1281977840648327292) is allowed to use
            // every command in every server, regardless of server permissions,
            // command category state, maintenance mode, cooldowns, or guild-level
            ...
            if (!isOwner && !isCommandCategoryEnabled(command.category)) {
              throw createError(
            ...
            if (!abuseProtection.allowed) {
              ...
              throw createError(
                ...
                withTraceContext({
                  commandName: interaction.commandName,
                  remainingMs: abuseProtection.remainingMs
                }, interactionTraceContext)
              );
            }
            ...
            if (interaction.guild) {
              ...
              if (!isOwner && !(await isCommandEnabled(
                client,
            ...
            // The permission guard already recognizes the bot owner, so the
            // owner bypasses default_member_permissions as well.
        ...
        await handleInteractionError(
          interaction,
          error,
          withTraceContext({ type: 'interaction', subtype: error?.context?.subtype }, interactionTraceContext)
        );
      }
    });
  },
