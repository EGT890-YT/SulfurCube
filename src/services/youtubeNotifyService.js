...
function normalizeHandle(input) {
  ...
  return value.replace(/^@/, '').replace(/^https?:\/\/(?:www\.)?youtube\.com\//i, '').split(/[/?#]/)[0];
}
...
export async function resolveYouTubeChannel(input) {
  ...
  try {
    response = await fetch(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SulfurCube/1.0)', 'Accept-Language': 'en-US,en;q=0.9' },
      redirect: 'follow',
  ...
  if (!channelId) throw new Error('I could not find that YouTube channel. Make sure the handle is correct.');
  const title = html.match(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i)?.[1] || `@${handle}`;
  return { handle: `@${handle}`, channelId, name: decodeXml(title), url };
}
...
async function fetchFeed(channelId) {
  ...
  try {
    response = await fetch(`${FEED_URL}${encodeURIComponent(channelId)}`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SulfurCube/1.0)', 'Accept': 'application/atom+xml, application/xml;q=0.9, */*;q=0.8' },
      signal: controller.signal,
    });
  } finally { clearTimeout(timeout); }
...
async function classifyVideo(videoId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`YouTube returned HTTP ${response.status}.`);
    const html = await response.text();
    const isLive = /"isLiveContent":true/i.test(html) || /"isLiveNow":true/i.test(html) || /"isLive":true/i.test(html);
    const isShort = /"isShorts":true/i.test(html) || /"canonicalBaseUrl":"\/shorts\//i.test(html) || /https:\/\/www\.youtube\.com\/shorts\//i.test(html);
    if (isLive) type = 'live';
    else if (isShort) type = 'shorts';
  } catch (error) {
    logger.warn(`Could not classify YouTube video ${videoId}: ${error.message}`);
  }
  classificationCache.set(videoId, { type, expiresAt: Date.now() + 10 * 60 * 1000 });
  return type;
}

function buildNotificationEmbed(video, channel, type) {
  const labels = { live: '🔴 LIVE STREAM', longform: '🎬 NEW VIDEO', shorts: '📱 NEW SHORT' };
...
export async function removeYouTubeNotifications(client, guildId, youtubeInput) {
  const config = await getGuildConfig(client, guildId);
  const subscriptions = Array.isArray(config.notifyUploads) ? config.notifyUploads : [];
...
export async function getConfiguredYouTubeHandles(client, guildId) {
  const config = await getGuildConfig(client, guildId);
  const subscriptions = Array.isArray(config.notifyUploads) ? config.notifyUploads : [];
  const seen = new Set();
  return subscriptions
    .filter(entry => entry.youtubeHandle)
    .filter(entry => {
      const key = String(entry.youtubeHandle).toLowerCase();
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    })
    .map(entry => ({ handle: entry.youtubeHandle, name: entry.youtubeChannelName || entry.youtubeHandle }));
}
