...
export const botConfig = {
  ...
  // =========================
  commands: {
    // Always include the configured owner as a fallback. This prevents the
    // recovery commands from becoming inaccessible when OWNER_IDS is missing
    // or stale in the hosting environment.
...
export function getColor(color) {
  if (!color) return 0x336699;
  if (typeof color === 'number') return color;
  if (typeof color !== 'string') return 0x336699;
  const normalized = color.replace(/^#/, '').trim();
  if (/^[0-9a-fA-F]{6}$/.test(normalized)) return parseInt(normalized, 16);
  const named = botConfig.embeds?.colors?.[color];
  if (typeof named === 'string') return getColor(named);
  return 0x336699;
}
