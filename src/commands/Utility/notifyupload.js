...
function normalizePing(input, guild) {
  ...
  if (roleMatch) {
    if (!guild.roles.cache.has(roleMatch[1])) {
      throw new Error('That role could not be found in this server.');
    }
    return value;
  }
  ...
  if (userMatch) {
    if (!guild.members.cache.has(userMatch[1])) {
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option
        .setName('ping')
        .setDescription('Who should be pinged? @everyone, @here, a role, or a person.')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('youtube_channel')
        .setDescription('YouTube handle, e.g. @example or youtube.com/@example')
        .setRequired(true)
    )
    .addChannelOption(option =>
      option
        .setName('channel')
        .setDescription('Discord channel where the notification will be sent.')
    ...
    .addStringOption(option =>
      option
        .setName('type')
        .setDescription('What should be announced?')
        .setRequired(true)
        .addChoices(
  ...
  async execute(interaction, config, client) {
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
      return interaction.reply({
        content: '❌ You need **Manage Server** to configure YouTube notifications.',
        ephemeral: true,
      });
    }

    const pingInput = interaction.options.getString('ping', true);
    ...
    try {
      ...
      if (current.length >= MAX_SUBSCRIPTIONS) {
        return interaction.reply({
          content: `❌ This server already has the maximum of **${MAX_SUBSCRIPTIONS}** YouTube notification channels configured.`,
          ephemeral: true,
        });
      }

      const ping = normalizePing(pingInput, interaction.guild);
      ...
      if (
        ...
        return interaction.reply({
          content: `❌ I need **View Channel**, **Send Messages**, and **Embed Links** in ${discordChannel}.`,
          ephemeral: true,
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Configured', value: `${total}/${MAX_SUBSCRIPTIONS}`, inline: true },
        )
